...
class InferencePrimitive(BaseModel):
    ...
    name: str
    category: str = Field(
        default="general", description="e.g. game_theory, system_dynamics, causal"
    )
...
class PrimitiveLibrary(BaseModel):
    ...
    def get(self, name: str) -> InferencePrimitive | None:
    ...
    def underperformers(
        self, min_uses: int = 5, threshold: float = 0.4
    ) -> list[InferencePrimitive]:
    ...
    @classmethod
    def default_library(cls, calibration: object | None = None) -> PrimitiveLibrary:
        """
